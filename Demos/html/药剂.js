...
function initializeTable() {
    ...
    $("#view4b77b5929d2d42a8b59f47495c21ade8").datagrid({
        url: configAddress + "/prescription/findChmPrescriptions", //page=1&rows=10初始化不变
        method: 'post',
...
function isHaveInfo(item) {
    ...
    if (item.footnote) {
        myObj.use = item.footnote;
    }
    if (item.dutyDoctor) {
        myObj.doctor = item.dutyDoctor;
        doctorStyle = 'display: inline-block;color: red;border: 1px solid red;padding: 0px 4px;'
    }
    return myObj;
        ...
        success: function (data) {
            ...
            if (data.error == true) {
                ...
                for (var j = 0; j < data.preDetail.length; j++) {
                    ...
                    if(data.data.preDetail[j].dutyDoctor){
                        preDetails += "<div class='drug_items'>"+
                        "<h1 style='display:inline-block;float:left'>"+data.preDetail[j].drugName+"</h1>"+
                        "<div class='drug_item_info'>"+
                            "<div class='drug_item_info_use'>"+isHaveInfo(data.preDetail[j]).use+"</div>"+
                            "<div class='drug_item_info_bottom'>"+
                            "<span class='drug_item_info_count'>"+data.preDetail[j].number+data.preDetail[j].numberCompany+"</span>"+
                            "<span class='drug_item_info_doctor'>"+isHaveInfo(data.preDetail[j]).doctor+"</span>"+
                            "</div>"+
                        "</div>"+
                        "</div>"
                    }else{
                ...
                let myHtml = "<div id='prescriptionLookHead'>" +
                    ...
                    "</div>" +
                    "</div>" +
                    "<div id='prescriptionLookDrugList'>"+
                    "<div id='drug_container' style='overflow:hidden'>"+
                    "<h1 style='display:inline-block;float:left'>R.</h1>" +
                    preDetails +
                    "</div>"+
                    "</div>"+
                ...
                //处方斜线样式设置
                let drugHeight =Number($('#drug_container').css('height').split('p')[0]);
                console.log(drugHeight);
                if(drugHeight == 86){
                    $("#prescriptionDivider").css({
                        "transform": "rotate(-23deg)",
                        "top": "380px"
                    })
                }else if(drugHeight == 172){
                    $("#prescriptionDivider").css({
                        "transform": "rotate(-14deg)",
                        "top": "425px"
                    })
                }else if(drugHeight == 258){
                    $("#prescriptionDivider").css({
                ...
                //判断样式
                if($('.drug_item_info_use').text() == ''){
                    $('.drug_item_info').css('margin-top','45px')
                }else{
                    $('.drug_item_info').css('margin-top','28px')
                }
            }
        },
        error: function (err) {
...
<div id='prescriptionLook' style="user-select:text;position: relative;">
    ...
    <script>
        ...
        function initializeTable() {
            ...
            $("#view8f7ad534f6ce431a9f2ab286891e8e70").datagrid({
                url: configAddress + "/prescription/findChmPrescriptions", //page=1&rows=10初始化不变
                method: 'post',
                height: $("#middle .middleTable").height(),
                autoRowHeight:false,
                ...
                success: function (data) {
                    ...
                    if(data.error == true){
                        ...
                        let myHtml = "<div id='prescriptionLookHead'>" +
                            ...
                            "<div id='prescriptionDivider'></div>" +
                            "<div id='Dosage'>"+
                               "<span>共计 "+data.total+" 剂</span>"+
                               "<span>"+data.dailyDose+"</span>"+
                               "<span>"+data.decoctionWay+"</span>"+
                               "<span>"+data.takeEntrust+"</span>"+
                               "<span>"+data.dayFrequency+"</span>"+
                            "</div>"+
                            "<div id='prescriptionLookFooter'>" +
                                "<div>" +
                                    "<div>医师：<span class='doctorName'>" + data.doctor + "</span></div>" +
